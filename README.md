...
| `NS_COMMENT_COUNT` | 否 | `20` | 每次评论多少个帖子，`0` 表示只签到不评论 |
| `NS_COMMENT_BATCH` | 否 | `3` | 每批评论几条 |
| `NS_COMMENT_INTERVAL` | 否 | `300` | 每批之间停几秒 |
| `NS_CHICKEN_COUNT` | 否 | `2` | 每天投喂几次（站点免费额度当前是 2） |
...
站点每天的鸡腿来源：签到 5 个、评论最多 20 个（1 条 1 个）、发帖最多 20 个、
免费投喂 2 次。脚本覆盖签到、评论、投喂三项。

评论默认 20 条，但**分批进行**：每 3 条休息 5 分钟，跑满约 40 分钟。连续
快速刷 20 条评论既容易触发站点风控，也更容易被人一眼看出是机器人。

候选帖子按目标数的 3 倍收集并支持翻页。实测交易区第一页 50 个帖子里有
相当一部分评论不了（老帖、已关闭回复、需要更高权限），不备足候选就凑不满
目标数；评论失败会自动跳过并补位。

### 定时任务排了三班

实测 GitHub 的定时触发很不准，这个仓库的延迟在 30 分钟到 8.5 小时之间浮动，
有一天更是整天没触发。所以排了三班：
...
签到接口能区分「本次签到收益」和「今天已完成签到」，后者说明当天已经跑过
一轮，此时会自动跳过评论，所以多排几班不会重复刷评论。手动触发不受此限制，
方便补跑。
